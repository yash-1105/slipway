...
async def test_a_successful_call_returns_the_text_and_usage() -> None:
    ...
    result = await client.complete(role="spec", messages=MESSAGES, timeout_seconds=5.0)
...
async def test_a_timeout_is_retried_and_then_returned_as_a_value() -> None:
    ...
    result = await client.complete(role="spec", messages=MESSAGES, timeout_seconds=0.01)
...
async def test_a_role_missing_from_the_catalogue_fails_before_any_call() -> None:
    ...
    result = await client.complete(role="build", messages=MESSAGES, timeout_seconds=5.0)
...
async def test_the_model_id_comes_from_the_catalogue_not_from_code() -> None:
    ...
    await client.complete(role="spec", messages=MESSAGES, timeout_seconds=5.0)
